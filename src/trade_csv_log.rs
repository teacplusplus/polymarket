...
// ---------------------------------------------------------------------------
// Submit-orders CSV: отдельный канал для submit-режима, фиксирует ровно одну
// строку на терминальное событие ордера (taker SELL match SL/Timeout/EvExit*
// или maker TP fill). По сравнению с базовым `TradeCsvRow`:
//   * Расширенная структура (plan vs actual цифры, все три order_id'а,
//     fill_role, finalized_via).
//   * Без буфера / `final_outcome` — submit-закрытие происходит до резолюции
//     маркета, исход `up_won` ещё не известен; если нужен — JOIN с
//     `last_real_sim_with_submit_trades.csv` по `market_id`.
// ---------------------------------------------------------------------------

/// Глобальный buffered writer per-order CSV для submit-режима. См. модульный
/// комментарий выше. `None` — лог не открыт, `write_submit_trade_csv_row` —
/// no-op (та же безопасность, что у [`TRADE_CSV_LOG`]).
pub static SUBMIT_TRADE_CSV_LOG: Mutex<Option<BufWriter<File>>> = Mutex::new(None);

/// CSV-заголовок submit-orders файла. Порядок строго совпадает с
/// [`write_submit_trade_csv_row_to_file`].
...
/// Открывает / перезаписывает submit-orders CSV-файл по пути `path`,
/// пишет заголовок. Симметрично [`init_trade_csv_log_file`].
pub fn init_submit_trade_csv_log_file(path: &Path) -> std::io::Result<()> {
    ...
    let file = File::create(path)?;
    let mut writer = BufWriter::new(file);
    writeln!(writer, "{}", SUBMIT_TRADE_CSV_HEADER)?;
    let mut guard = SUBMIT_TRADE_CSV_LOG
        .lock()
        .expect("SUBMIT_TRADE_CSV_LOG poisoned");
    *guard = Some(writer);
    Ok(())
}

/// Флашит и закрывает submit-orders writer. Симметрично [`finish_trade_csv_log`].
pub fn finish_submit_trade_csv_log() {
    if let Ok(mut guard) = SUBMIT_TRADE_CSV_LOG.lock()
...
/// Полный набор колонок для одной submit-закрытой сделки.
///
/// * `pos_id` — локальный uuid из [`crate::history_sim::OpenPosition::id`];
///   корреляция с tee-логами submit-флоу (grep `pos_id=<uuid>`).
/// * `planned_*` vs живые `buy_price` / `shares_held` / `entry_cost` —
///   plan vs actual; см. doc у `OpenPosition::planned_shares_held`.
/// * `open_order_id` / `tp_order_id` / `close_order_id` — все три CLOB id
///   жизненного цикла позиции (BUY → TP-place → SELL).
/// * `fill_role` — `Taker` для SELL-close из `spawn_close_via_taker`,
///   `Maker` для TP-fill'а (мы maker).
/// * `finalized_via` — `Ws` если финализация пришла по user-WS trade event,
///   `Polling` если по REST-fallback'у из `account_submit::drive_*`.
#[derive(Debug, Clone, Copy)]
pub struct SubmitTradeCsvRow<'a> {
    pub pos_id: &'a str,
    pub polymarket_url: &'a str,
    pub price_to_beat: Option<f64>,
    pub final_price: Option<f64>,
    ...
    pub planned_buy_price: f64,
    pub buy_price: f64,
    pub planned_shares_held: f64,
    pub shares_held: f64,
    pub planned_entry_cost: f64,
    pub entry_cost: f64,
    pub exit_price: f64,
    pub fee_usdc: f64,
    pub pnl: f64,
    ...
    pub close_order_id: Option<&'a str>,
    pub raw_pred: f32,
    pub cal_pred: f32,
    pub kelly_f: f64,
    pub p_win_ema_at_close: Option<f64>,
...
/// Пишет одну строку submit-orders CSV напрямую в файл (без буфера). Если
/// файл не открыт, — no-op. На `NaN`/`Inf` числовые ячейки пишутся пустыми
/// (см. [`fmt_f64`] / [`fmt_f32`]).
pub fn write_submit_trade_csv_row(row: SubmitTradeCsvRow<'_>) {
    let regime: &'static str = CURRENT_REGIME.lock().map(|g| *g).unwrap_or("");
    ...
    write_submit_trade_csv_row_to_file(w, regime, &row);
    let _ = w.flush();
}

/// Сериализатор одной [`SubmitTradeCsvRow`]; порядок колонок строго
/// соответствует [`SUBMIT_TRADE_CSV_HEADER`].
fn write_submit_trade_csv_row_to_file(
    w: &mut BufWriter<File>,
    regime: &'static str,
    row: &SubmitTradeCsvRow<'_>,
) {
    let _ = writeln!(
        w,
        "{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{}",
        ...
        row.fill_role,
        row.finalized_via,
        fmt_f64(row.planned_buy_price),
        fmt_f64(row.buy_price),
        fmt_f64(row.planned_shares_held),
        fmt_f64(row.shares_held),
        fmt_f64(row.planned_entry_cost),
        fmt_f64(row.entry_cost),
        fmt_f64(row.exit_price),
        fmt_f64(row.fee_usdc),
        fmt_f64(row.pnl),
        ...
        row.close_order_id.map(csv_escape).unwrap_or_default(),
        fmt_f32(row.raw_pred),
        fmt_f32(row.cal_pred),
        fmt_f64(row.kelly_f),
        row.p_win_ema_at_close.map(fmt_f64).unwrap_or_default(),
