...
pub(crate) async fn redeem_x_entry_size(
    ...
    //   * пока своя нога открыта, а ПАРТНЁР ПУСТ (own>0, other=0) — соло-ногу НЕ доливаем: без
    //     партнёра это чистая направленная ставка, которая при разбегании цены сливает ногу
    //     целиком (см. solo-leg gate ниже); ждём появления второй ноги;
    ...
    if other_shares > 0.0 {
    ...
    } else if own_shares == 0.0 {
        // Рынок ещё НЕ открыт (ОБЕ ноги пусты). Воркеры up/down спавнятся параллельно на все
        // лейны (см. real_sim::LANE_FRAME_ROUTES), поэтому без этого гейта на старте открылись
        // бы СРАЗУ обе ноги. Открываем рынок только ДЕШЁВОЙ стороной — андердогом
        ...
        // включится логика пары (см. ветку выше) — как только у андердога появятся шеры.
        if prob >= other_prob {
            crate::tee_println!(
        ...
        // андердог — разрешаем первый клип рынка (падаем к гейтам экспозиции/банка ниже).
    } else {
        // Своя нога уже открыта, а партнёр ПУСТ (own_shares > 0, other_shares == 0). НЕ доливаем
        // соло-ногу: именно безпартнёрный разгон одной стороны слил всю ногу (-64.73 на рынке
        // 0x321edc92 — андердог открылся у ~0.46, цена убежала, партнёр так и не встал, т.к.
        // пара пробивала cap, а мы догнали ногу до ~298 шт и потеряли её целиком на резолюции).
        // pair-логика выше авторизует докорм только при НЕПУСТОМ партнёре (улучшение worst-case
        // или усреднение пары вниз) — до его появления держим только уже открытые клипы. Кэп
        // экспозиции ($300) тут не защищает: дешёвую ногу можно набрать на сотни штук в лимите.
        crate::tee_println!(
