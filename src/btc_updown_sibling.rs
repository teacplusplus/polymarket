//! Пара BTC up/down 5m ↔ 15m: длительности окон и состояние sibling-маркетов для [`crate::project_manager::ProjectManager`].
...
#[derive(Debug, Clone)]
pub struct BtcUpdownSiblingSlot {
    pub market_id: String,
    /// Старт окна из текста Gamma `question` (ET → Unix-сек), см. [`crate::gamma_question::btc_updown_question_window_start_unix_sec`].
    pub window_start_sec: i64,
}

/// Активные маркеты по WS: один слот 15m и один 5m; [`on_btc_updown_ws_connected`] обновляет слот перед запуском WS.
#[derive(Debug, Default)]
pub struct BtcUpdownSiblingWsState {
    pub fifteen_min: Option<BtcUpdownSiblingSlot>,
    pub five_min: Option<BtcUpdownSiblingSlot>,
}

impl BtcUpdownSiblingWsState {
...
/// В слотах сохраняется старт окна из [`crate::gamma_question::btc_updown_question_window_start_unix_sec`].
/// Согласованную пару для merge sibling-признаков читает [`BtcUpdownSiblingWsState::paired_five_and_fifteen_market_ids`].
pub async fn on_btc_updown_ws_connected(
    btc_updown_sibling_ws_state: Arc<RwLock<BtcUpdownSiblingWsState>>,
    interval_sec: i64,
    ...
    let Some(question) = gamma_question.filter(|s| !s.is_empty()) else {
        eprintln!("btc_updown_sibling: пустой gamma_question — слот не обновлён");
        return;
    };

    let Some((parsed_start_unix, duration_min)) =
        btc_updown_question_window_start_unix_sec(question, slug_window_start_unix_sec)
    else {
        eprintln!("btc_updown_sibling: не разобрать окно из question для market_id={condition_id}");
        return;
    };

    let Some(horizon) = BtcUpdownInterval::try_from_interval_sec(interval_sec) else {
        return;
    };
    let expected_dur_min = horizon.duration_minutes();
    if duration_min != expected_dur_min {
        eprintln!("btc_updown_sibling: в question длительность {duration_min} мин, ожидалось {expected_dur_min} для interval_sec={interval_sec} (market_id={condition_id})");
        return;
    }
    ...
    match horizon {
        ...
        BtcUpdownInterval::FiveMin => {
            ...
            btc_updown_sibling_ws_state_lock.five_min = Some(BtcUpdownSiblingSlot {
                market_id: condition_id,
                window_start_sec: parsed_start_unix,
            });
        }
